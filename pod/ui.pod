...
For first time users of CF+, consider skimming through the
...
This section contains some of the tooltips of the Crossfire+ client.
...
B<Speed>, how fast you can move. The value roughly means how many actions
per second you can make. It may range between nearly 0 ("very slow") to
higher than 5 ("lightning fast"). Base speed is determined from the Dex
and modified downward proportionally by the amount of weight carried which
exceeds the Max Carry limit. The armour worn also sets the upper limit on
speed.
